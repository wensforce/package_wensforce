...
export default function CreateTripModal({
  ...
  const [debouncedAdditionalServiceQuery, setDebouncedAdditionalServiceQuery] = useState("");
  ...
  const [selectedAdditionalServices, setSelectedAdditionalServices] = useState(new Map());
  ...
  useEffect(() => {
    ...
    if (trip) {
      ...
      if (Array.isArray(trip.additionalServices) && trip.additionalServices.length > 0) {
        const map = new Map(
          trip.additionalServices.filter((s) => s?.id).map((s) => [Number(s.id), s]),
        );
  ...
  useEffect(() => {
    ...
    if (!query) { setSubscriptionOptions([]); setLoadingSubscriptions(false); return; }
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    if (!query) { setUserOptions([]); setLoadingUsers(false); return; }
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    servicesApi
      .servicesNotIncluded(selectedSubscription.packageId, {
        ...
        limit: 10,
        ...(debouncedAdditionalServiceQuery ? { search: debouncedAdditionalServiceQuery } : {}),
      })
      .then((res) => {
        if (cancelled) return;
      ...
      .catch(() => { if (!cancelled) setAdditionalServicesOptions([]); })
      .finally(() => { if (!cancelled) setLoadingAdditionalServices(false); });
    return () => { cancelled = true; };
  ...
  function validate() {
    ...
    if (!selectedServiceIds.size) return "Please select at least one included service.";
    return null;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const additionalServices = Array.from(selectedAdditionalServices.values()).map((svc) => ({
        id: Number(svc.id),
        name: svc.title || svc.name || `Service #${svc.id}`,
      }));
  ...
  // ── Derived UI values ─────────────────────────────────────────────────────
  const allChecked =
    packageServices.length > 0 && selectedServiceIds.size === packageServices.length;
  const someChecked =
    selectedServiceIds.size > 0 && selectedServiceIds.size < packageServices.length;
            ...
            <div className="rounded-xl border border-[#CBD5E0] bg-[#FAF6EC] p-3 space-y-3">
              ...
              {selectedSubscription && (
                <div className="inline-flex items-center gap-2 rounded-full border border-[#CBD5E0] bg-white px-3 py-1.5 text-xs text-[#1A202C]">
                  <Repeat size={12} />
                  <span className="max-w-[160px] truncate" title={`Subscription #${selectedSubscription.id}`}>
                    Sub #{selectedSubscription.id}
                    {selectedSubscription.package?.name ? ` · ${selectedSubscription.package.name}` : ""}
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                ...
                {loadingUsers ? (
                  ...
                  userOptions.map((user) => (
                    <button
                      key={user.id}
                      type="button"
                      onClick={() => { setSelectedUser(user); setUserSearch(""); setUserOptions([]); }}
                      disabled={submitting}
                      className="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-left text-sm text-[#1A202C] hover:bg-[#FAF6EC] transition-colors disabled:opacity-60"
                    >
                      <span className="truncate">
                      ...
                      {packageServices.map((service) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="flex-1 text-sm text-[#1A202C] truncate min-w-0" title={label}>
                              {label}
                            </span>
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <span className="text-xs text-[#A0AEC0]">#{service.id}</span>
                            </div>
                          </label>
                        );
                      })}
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#718096] font-normal">(not in package)</span>
            </div>
              ...
              <>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setAdditionalServiceQuery(e.target.value)}
                ...
                {loadingAdditionalServices ? (
                  <div className="flex items-center gap-2 text-xs text-[#4A5568] px-1 py-2">
                    <Loader2 size={14} className="animate-spin" /> Loading additional services…
                  </div>
                        ...
                        return (
                          <label
                            ...
                            className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-colors select-none ${
                              isSelected ? "bg-[#FAF6EC]" : "hover:bg-[#FAF6EC]"
                            }`}
                          >
                            <input
                              type="checkbox"
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              {price ? (
                                ...
                                <span className="text-xs text-[#A0AEC0] italic">No price</span>
                              )}
                              <span className="text-xs text-[#A0AEC0]">#{service.id}</span>
                            </div>
                          </label>
                  ...
                  <div className="rounded-lg border border-[#C9A24B]/30 bg-white overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-3 py-2 border-b border-[#EDF2F7] bg-[#FFFDF5]">
                      ...
                      <span className="text-xs text-[#718096]">
                        {selectedAdditionalList.length} service{selectedAdditionalList.length > 1 ? "s" : ""}
                      </span>
                    </div>
                    ...
                    <div className="divide-y divide-[#EDF2F7]">
                      {selectedAdditionalList.map((svc) => {
                        const label = svc.title || svc.name || `Service #${svc.id}`;
                        ...
                        return (
                          ...
                          >
                            <ConciergeBell size={12} className="text-[#C9A24B] shrink-0" />
                            <span className="flex-1 text-sm text-[#1A202C] truncate min-w-0" title={label}>
                              {label}
                            </span>
                            <div className="flex items-center gap-2 shrink-0">
