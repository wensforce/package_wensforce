...
function Row({ label, value, mono = false }) {
  return (
    <div>
      ...
      <p className={`text-sm text-[#1A202C] ${mono ? "font-mono" : "font-medium"}`}>{value}</p>
    </div>
  );
}
    ...
    <div className="bg-white rounded-2xl border border-[#CBD5E0] p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-2xl font-bold text-[#0B1E3F]">{user.name || "Unnamed User"}</h2>
      ...
      >
        {orders.length === 0 ? (
          <p className="text-sm text-[#4A5568]">No orders found for this user.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#FAF6EC] border-b border-[#CBD5E0]">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Amount</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Final</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Status</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Created</th>
              ...
              <tbody className="divide-y divide-[#CBD5E0]">
                {orders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-[#FAF6EC]/70 transition-colors">
                      <td className="px-4 py-3 text-xs font-mono text-[#0B1E3F]">#{order.id}</td>
                      <td className="px-4 py-3 text-xs text-[#4A5568]">{formatMoney(order.amount)}</td>
                      <td className="px-4 py-3 text-xs font-semibold text-[#0B1E3F]">{formatMoney(order.finalAmount)}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${status.className}`}>
                          {status.icon} {status.label}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs text-[#4A5568] whitespace-nowrap">{formatDate(order.createdAt)}</td>
                      <td className="px-4 py-3">
                        <button
      ...
      >
        {subscriptions.length === 0 ? (
          <p className="text-sm text-[#4A5568]">No subscriptions found for this user.</p>
        ) : (
          <div className="overflow-x-auto rounded-xl border border-[#CBD5E0] bg-white">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#FAF6EC] border-b border-[#CBD5E0]">
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">#</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Package</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Trips</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Status</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#4A5568]">Open</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#CBD5E0]">
                {subscriptions.map((sub) => {
                  ...
                  return (
                    <tr key={sub.id} className="hover:bg-[#FAF6EC]/70 transition-colors">
                      <td className="px-4 py-3 text-xs font-mono text-[#0B1E3F]">#{sub.id}</td>
                      <td className="px-4 py-3 text-xs text-[#4A5568]">#{sub.packageId}</td>
                      <td className="px-4 py-3 text-xs text-[#4A5568]">{sub.tripsUsed ?? 0} / {sub.tripsTotal ?? 0}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${status.className}`}>
                          {status.icon} {status.label}
                        </span>
