...
export default function PackageForm({ packageId, initialData, onSaved }) {
    ...
    serviceSearchTimeouts.current[index] = setTimeout(async () => {
      try {
        const res = await api.get("/service/list", {
          params: { page: 1, limit: 5, search: value.trim() },
        });
        const services = res.data?.data?.services ?? [];
        setServiceSuggestions((prev) => ({ ...prev, [index]: services }));
      } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (thumbnail) {
        const uploadResult = await uploadImageToS3(thumbnail);
      ...
      if (isEditMode) {
        await api.put(`/package/${packageId}`, payload);
      } else {
      ...
      if (uploadedKey) {
        await rollbackS3Upload(uploadedKey);
      }
              ...
              {form.services.map((serviceItem, index) => (
                ...
                >
                  ...
                  <div className="flex flex-col space-y-1.5">
                    {/* ✅ Invisible spacer — matches label height so button
            always lines up with the inputs regardless of whether
            the "Selected:" badge is shown or not              */}
                    <span
