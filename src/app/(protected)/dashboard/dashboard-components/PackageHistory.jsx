...
import { setPackageHistory } from "../slices/package-history-slice";
...
function StatCell({ icon, label, value }) {
  return (
    <div className="flex flex-col items-center gap-0.5 min-w-0 font-sans">
      <div
    ...
    >
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="flex-1 p-5 flex flex-col gap-4 min-w-0">
          ...
          {/* Stats row */}
          <div
            className="flex items-center justify-between rounded-xl px-3 py-3 gap-2 flex-wrap sm:flex-nowrap"
            style={{
          ...
          {/* Footer */}
          <div className="flex items-center justify-between pt-1 mt-auto gap-4 flex-wrap">
            <p
              className="text-xs"
              style={{ color: "var(--color-text-tertiary)" }}
            >
              {plan.status === "pending" ? "Purchased on: " : "Valid till: "}
              <span
...
export default function PackageHistory() {
  ...
  const storePlans = useSelector((state) => state.packageHistory.value);
  ...
  async function fetchFromApi({ isRefresh = false } = {}) {
    ...
    try {
      const data = await subscriptionApiUser.getMySubscriptionHistory();
      const fetchedPlans = normalisePlans(data?.data);
      setPlans(fetchedPlans);
      dispatch(setPackageHistory(fetchedPlans));
      if (isRefresh) toast.success("Membership history refreshed");
    } catch (err) {
      console.error(err);
      toast.error("Failed to load membership history. Please try again.");
