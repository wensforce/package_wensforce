      ...
      {units.map(({ label, val }, i) => (
        <div key={label} className="flex items-center gap-2 sm:gap-3">
          <div className="flex flex-col items-center">
            ...
            <span className="text-[9px] text-white/30 uppercase tracking-[0.15em] mt-1.5 font-medium">{label}</span>
          </div>
          {i < 3 && (
...
export default function FoundingMemberBanner() {
  ...
  return (
    ...
    >
      ...
      {/* Top glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-48 rounded-full blur-3xl pointer-events-none" style={{ background: 'radial-gradient(ellipse, rgba(201,162,75,0.18) 0%, transparent 70%)' }} />
      ...
      <div className="relative max-w-3xl mx-auto">
        ...
        {/* Divider */}
        <div className="w-full h-px mb-10" style={{ background: 'linear-gradient(90deg, transparent, rgba(201,162,75,0.3), transparent)' }} />
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-5 gap-2 sm:gap-3">
            {tierList.map((id) => (
              ...
              >
                <p className="text-[9px] text-white/40 uppercase tracking-wide font-light text-center">{id}</p>
                <p className="text-[11px] font-semibold text-[#C9A24B] text-center">{TIER_PRICES[id]}</p>
        ...
        <div
          ...
          style={{ background: 'rgba(201,162,75,0.07)', border: '1px solid rgba(201,162,75,0.15)' }}
        >
          <p className="text-[#C9A24B] text-[10px] uppercase tracking-[0.3em] font-semibold mb-3">What you get as a founding member</p>
            ...
            {[
              ...
              <div key={title} className="flex items-start gap-3">
                <div className="mt-0.5 w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: 'rgba(201,162,75,0.15)' }}>
                  <Icon size={13} strokeWidth={1.75} className="text-[#C9A24B]" />
                </div>
                <div>
                  <p className="text-white/70 text-xs font-semibold leading-tight mb-0.5">{title}</p>
                  <p className="text-white/30 text-[11px] font-light leading-snug">{desc}</p>
                </div>
              </div>
            ))}
          </div>
